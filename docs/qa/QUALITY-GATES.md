...
|---|---|---|
| **Fixtures bypass the API to create state the product cannot create** — *partly closed 2026-07-19* | `tests/e2e/global-setup.mjs` and `apps/api/test/scoring-flow.test.mjs:114` still call `performanceAgreement.create` directly. **Identity rows are no longer a gap:** CCR-014 added the onboarding operations and `tests/e2e/onboarding.spec.ts` now creates a working teacher account through the browser with no Prisma write in the path | The remaining half is `SEIP-BLOCK-002`, whose acceptance criteria require deleting the raw agreement insert. The one identity still created outside the API is the bootstrap admin — an operator CLI by design (CCR-014 decision 1), not a fixture shortcut |
| ~~**Permission sweep covers 28 of 31 matrix operations**~~ — **closed 2026-07-19** | Sweep now covers **34 of 35** rows (`permission-matrix.test.mjs`, 204 assertions). `completeFileUpload` and `getEvidenceFileDownloadUrl` were added and immediately reported 3 mismatches — all three were the sweep not knowing those ops are ownership-gated, not enforcement bugs | The single remaining exclusion is `getAssignmentResults`, excluded *deliberately* with a documented reason and covered by `scoring-flow.test.mjs:333` |
