...
25. **Sprint 2 — BLOCKERS (must precede any release; each needs a CCR + a version bump — major vs minor decided per CCR, not assumed).** Tracked as `SEIP-BLOCK-001/002/003` on `.ai-team/task-board.yaml` (CCR-014/015/016). **CCR-014 drafted 2026-07-19, awaiting approval** — it lands additive (openapi 2.8.0 → 2.9.0), correcting this entry's original claim that all three require a major bump:
    ...
    - **B-2 PerformanceAgreement + AgreementChallenge** (`SEIP-BLOCK-002`, CCR-015 drafted 2026-07-19)**.** Both models have **zero references** in `apps/` and `packages/`. Two consequences: `submitMyScores` always fails the workload gate (`apps/api/src/routes/scoring.ts:220-227`, VAL-002/SCORE-004), so **no score can be submitted at all**; and — correcting this entry's original claim that ประเด็นท้าทาย "has no code" — the 40% **is** computed correctly, but the committee scores it **blind**, because `AgreementChallenge` holds the method plan and targets that T-C.1/T-C.2.1/T-C.2.2 rate and nothing writes or reads it. CCR-015 also removes `AssignmentCreate.agreement_id` (breaking → openapi 3.0.0): the server can derive it, so the client should not supply it.
